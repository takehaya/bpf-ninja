...
| `--raw-frame-id` | Put the hook's raw packet identity (a kernel address: XDP `data_hard_start`, skb pointer) in `epb_packetid` / raw-dump instead of the opaque id. Research use only | entry, exit |
