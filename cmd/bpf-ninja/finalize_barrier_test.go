		...
		t.Run(fmt.Sprintf("fast=%v", fast), func(t *testing.T) {
			...
			send := func(shard int, tag int32) {
				p, e := ebpf.NewProgram(&ebpf.ProgramSpec{Type: ebpf.XDP, License: "GPL", Instructions: asm.Instructions{
					asm.LoadMapPtr(asm.R1, maps[shard].FD()), asm.Mov.Imm(asm.R2, 32), asm.Mov.Imm(asm.R3, 0), asm.FnRingbufReserve.Call(), asm.JEq.Imm(asm.R0, 0, "exit"),
					asm.Mov.Imm(asm.R2, 0), asm.StoreMem(asm.R0, 0, asm.R2, asm.DWord), asm.StoreMem(asm.R0, 8, asm.R2, asm.DWord), asm.StoreMem(asm.R0, 16, asm.R2, asm.DWord), asm.StoreMem(asm.R0, 24, asm.R2, asm.DWord), asm.StoreImm(asm.R0, 14, 4, asm.Half), asm.StoreImm(asm.R0, 16, int64(tag), asm.Word),
