...
func TestBpfSetFilterLookupAndRuntimeUpdate(t *testing.T) {
	...
	if err := def.Add(map[string]string{"imsi": fmt.Sprintf("%d", inImsi), "teid": fmt.Sprintf("%d", inTeid)}, 1, 0); err != nil {
		t.Fatalf("Add: %v", err)
	}
	...
	if err := def.Add(map[string]string{"imsi": fmt.Sprintf("%d", newImsi), "teid": fmt.Sprintf("%d", inTeid)}, 2, 0); err != nil {
...
func TestBpfSetFilterScalarKey(t *testing.T) {
	...
	if err := def.Add(map[string]string{"imsi": fmt.Sprintf("%d", member)}, 1, 0); err != nil {
		t.Fatalf("Add: %v", err)
	}
