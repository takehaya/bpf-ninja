...
func TestBpfShardsUnderRestrictedAffinity(t *testing.T) {
	...
	insns = append(insns, emitShardedRBReserve(outer.FD(), 0, int32(capture.MetadataSize+4))...)
	insns = append(insns, asm.LoadMem(asm.R1, asm.R10, -16, asm.Word), asm.StoreMem(asm.R0, capture.MetadataSize, asm.R1, asm.Word), asm.Mov.Imm(asm.R2, 0), asm.StoreMem(asm.R0, 0, asm.R2, asm.DWord), asm.StoreMem(asm.R0, 8, asm.R2, asm.DWord), asm.StoreMem(asm.R0, 16, asm.R2, asm.DWord), asm.StoreMem(asm.R0, 24, asm.R2, asm.Word), asm.StoreImm(asm.R0, 14, 4, asm.Half), asm.Mov.Reg(asm.R1, asm.R0), asm.Mov.Imm(asm.R2, 0), asm.FnRingbufSubmit.Call(), asm.Mov.Imm(asm.R0, 2).WithSymbol("exit"), asm.Return())
	...
	for _, fast := range []bool{false, true} {
		t.Run(strconv.FormatBool(fast), func(t *testing.T) {
			...
			sink := func(shard int, rec []byte) error {
				if len(rec) != capture.MetadataSize+4 || binary.NativeEndian.Uint32(rec[capture.MetadataSize:]) != uint32(shard) {
