...
var FilterSet = []FilterSpec{
	...
	{ID: "F2", Expr: "eth/ipv4[src==10.0.0.0/8]/tcp where tcp.dport == 80",
		...
		WantInsns: 210, Notes: "IPv4 source CIDR via bracket predicate"},
	...
	{ID: "F9", Expr: "eth/ipv4@outer/udp/geneve/eth/ipv4@inner/tcp where inner.dst == 10.0.0.1",
		WantInsns: 349, Notes: "Geneve inner IPv4 dst (Cilium overlay / cloud VPC); inner offset resolved past the opt_len*4 options section via the counter-driven bulk-advance fallback"},
	{ID: "F10", Expr: "eth/ipv4/tcp where tcp.options.MSS.value == 1460",
		WantInsns: 228, Notes: "TCP option MSS via parser-counter walk"},
}
