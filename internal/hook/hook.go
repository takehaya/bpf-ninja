...
type Hook struct {
	...
	// The filter body leaves R6..R9 untouched. R8 is never read after
	// the prologue on the tracing path (filters scan a scratch copy
	...
	// into dst from the ctx in R6 (valid anywhere after PacketPrologue).
	// The identity is the address of the buffer the packet sits in: it
	// is unchanged between the entry and exit of one invocation (XDP's
	// data_hard_start is not moved by adjust_head; an sk_buff object is
	// not replaced while its program runs). The gated capture uses it
	// in-kernel to check that the hold slot written at entry belongs to
	// the current invocation; it never leaves the kernel. Must not
	// clobber R0 or R6..R9.
