...
the shape of its states when it loads the protocol definition. A loop
qualifies as a counted aux-list walk
...
classification: the protocol definition declares the bottom-of-stack
sentinel as a chain-end constant, and the quantifier bound caps the
walk. The compiler rejects a multi-state parser cycle matching
