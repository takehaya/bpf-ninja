...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.85\columnwidth]{figures/fig_layerstart.pdf}
...
rather than the protocol. The choice is made when the protocol
definition is loaded, by the shape of its parser states rather than by
any per-protocol rule. A loop qualifies as a counted aux-list walk
...
optionally with a byte-count counter as in IPv4 options and Geneve. A
multi-state parser cycle matching neither shape is rejected at load
time rather than compiled to a wrong walk.
