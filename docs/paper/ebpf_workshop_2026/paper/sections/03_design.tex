...
\kunai evaluates the inner condition as a whole
against one element at a time. \texttt{any(P and Q)} therefore holds
only when a single element satisfies both \texttt{P} and \texttt{Q},
and \texttt{all()} applies the same element-wise rule. A conjunction
of separate quantifiers, \texttt{any(P) and any(Q)}, expresses the
other reading, where different elements may satisfy each predicate.
