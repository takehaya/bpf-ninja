...
protocol definitions written in a subset of P4, and a code-generation
stage that combines both to produce verifier-accepted eBPF bytecode.
\figref{fig:arch} shows the overall structure.

The compiler first validates the
...
split, \kunai handles filter analysis and bytecode generation, and
the host handles context access and follow-on actions such as
capture or drop.

\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{figures/fig_arch.pdf}
...
an excerpt of the grammar, and the full grammar is available in the
project repository.\footnote{\url{https://github.com/takehaya/bpf-ninja/tree/v0.23.1/pkg/kunai}}
...
A field reference inside a bracket predicate is relative to that
layer, and \texttt{where}-clause references name the layer
explicitly.
...
equals \texttt{10.0.0.1}. Here \texttt{@outer} and \texttt{@inner}
distinguish the two IPv4 layers. Because \texttt{inner.dst} uniquely
names the destination field of the inner IPv4 header, \kunai can
determine which field to read even though the same \texttt{ipv4}
...
\texttt{any(E)} holds when at least one element of the list satisfies
the whole inner condition \texttt{E}, and \texttt{all(E)} holds when
all elements do, where \texttt{E} may combine comparisons with
\texttt{and}, \texttt{or}, and \texttt{not}. \texttt{any(P and Q)}
therefore holds only when a single element satisfies both \texttt{P}
and \texttt{Q}. A conjunction of separate quantifiers, \texttt{any(P)
and any(Q)}, expresses the other reading, where different elements may
satisfy each predicate.

The walk requires a field giving the
element count, the width of each element, a way to advance to the next
element, and a stop condition, all of which the protocol definition
supplies. TCP options and GTP extension headers are handled the same
way when the protocol definition carries this information.
...
\begin{figure}[t]
\begin{lstlisting}[basicstyle=\scriptsize\ttfamily,breaklines=true,columns=fullflexible]
// headers, extern ParserCounter,
// const SRV6_ROUTING_TYPE elided
