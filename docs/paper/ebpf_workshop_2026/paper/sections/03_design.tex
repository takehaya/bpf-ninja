...
The chain before \texttt{where} lists the layers to traverse, and the
\texttt{where} clause gives the field condition checked once the chain
matches. Because \texttt{@outer} and \texttt{@inner} distinguish the
two IPv4 layers, \texttt{inner.dst} uniquely names the inner
destination field, and \kunai can determine which field to read even
