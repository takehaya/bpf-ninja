...
\author{Takeru Hayasaka$^{1,2,3}$ \quad Satoshi Uda$^{1}$ \quad Ayako Hayasaka$^{4}$ \quad Daisuke Kotani$^{5}$}
\affiliation{%
  \institution{$^{1}$Japan Advanced Institute of Science and Technology \quad $^{2}$BBSakura Networks Inc. \quad $^{3}$SAKURA internet Inc.\\
  $^{4}$Independent Researcher \quad $^{5}$Kyoto University}
  \country{}}
...
also requires bytecode that the Linux eBPF verifier accepts, and a
naive lowering is rejected. We present \kunai, a packet-filter DSL
