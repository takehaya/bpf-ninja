...
## Lexer の識別子 mode と value mode の切り替え
...
## Parser の再帰下降と precedence climbing
...
quantifier は `?` `+` `*` のいずれか、または `{n,m}` の range です。`{n}` は `{n,n}` と等価です。上限を省略した `{n,}` は open upper bound となり `RangeMax = -1` になります。これらも許可されます。

alternation `(a|b|c)` は layer-level の OR です。chain 中で IPv4 でも IPv6 でもよいという条件を 1 layer slot として扱えます。alt 直後の layer の dispatch const は後の resolver が alt ごとに収集し、alt 間で揃っていなくても解決できます。bracket predicate が alt-group 単位で課されるのは MVP 制約です。
...
- `ipv4@outer` のような explicit label は user が指定します。
- `ipv4#0`, `ipv4#1` のような auto index は、同 chain で 2 個目以降の同 protocol に自動で振られます。
...
## Error reporting と PositionedError
