...
quantifier は `?` `+` `*` のいずれか、または `{n,m}` の range です。`{n}` (= `{n,n}`) も許可されます。`{n,}` はエラーになります。上限なしは `+` で表現します。
...
子 layer の親 layer への dispatch を解決します。選択肢は 4 つあります。
...
resolver は `vocab.SelectDispatchConst(parentName)` で Field / NoCheck の優先順位で探し、nil なら `spec.IsSelfValidating()` で子の parser block が自己検証可能かを query します。結果がすべて nil なら `no dispatch constant for "foo" under "udp"` エラーになります。
...
aux ref については、parser block の `out` 引数として宣言された auxiliary header を vocab loader が分析して `AuxLayout` を populate し、resolver がそれを引いて FieldRef.Aux に詰めます。4 段階の path は次のとおりです。
