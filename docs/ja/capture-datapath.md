...
影響の向きは一方向です。`synchronize_rcu()` で眠るのは Put を発行した xdp-ninja のセットアップスレッドだけで、XDP のパケット処理を止めたり遅くしたりはしません。実測でも、セットアップがハングしている間、対象のデータパスは無損失で転送を続けていました。トラフィック負荷がセットアップを遅くすることはあっても、この更新がトラフィックへ負荷を与えることはありません。
