...
| パターン | 型 | 意味 |
|---|---|---|
| `<SELF>_<PARENT>_<FIELD>` | `bit<N>` | Field dispatch。親の `field` がこの値のとき自分として展開する |
| `<SELF>_<PARENT>_NO_CHECK` | `bool`、true 固定 | NoCheck dispatch。検査せず blind cast する |
...
```p4
const bool ETH_VXLAN_NO_CHECK = true;   // VXLAN inner Ethernet
const bool MPLS_MPLS_NO_CHECK = true;   // MPLS label stack (区切り無し)
```
...
`<SELF>_<SELF>_*` は `+` / `*` / `{n,m}` の 2 周目以降で読まれます。

```p4
const bit<16> VLAN_VLAN_ETHERTYPE = 0x8100;  // inner VLAN は ethertype 再一致で識別
const bool    MPLS_MPLS_NO_CHECK  = true;    // label に区切りが無いので blind
```
...
case 値は整数 literal、wildcard の `_`、bool のいずれかで、literal のみ使えます。シンボル名や範囲 match は書けないため、識別値は srv6.p4 の `4: skip_segments;` のように、コメントで RFC を引きながら inline で書きます。
...
| 一括スキップと index 参照で扱う固定間隔の要素列 | §7.4 declare-only stack | srv6 segments |
...
- mask は verifier 対策を兼ねます。`& 0x0F` のように上限を抑えておくと、verifier が静的上限を証明できます。srv6.p4 と ipv6.p4 のコメントを参照してください。
...
vocab 側は const だけで済み、parser block は trivial のままにします。self-dispatch の `MPLS_MPLS_NO_CHECK`、`MPLS_MAX_DEPTH`、`MPLS_CHAIN_END_S` を §5 のとおり宣言します。ユーザーが `mpls+` や `mpls{1,3}` と書いたときだけ bpf_loop に落ちます。
...
### 7.4 SRv6 型の declare-only stack

parser は要素を 1 個ずつ extract せず、可変長領域を §7.1 の advance で一括スキップします。スタックは宣言だけして、`srv6.segments[0].addr` のような DSL の index 参照のために layout を annotation で教えます。
...
- push されない top-level スタックには `@kunai_layout` が必須です。無いと loader が reject します。複数スタックが同じ offset に alias する事故を防ぐためです。
- `any` / `all` quantifier に実行時の要素数を教えるのが `@kunai_stack_count` です (§8)。
...
| `@kunai_layout` | parser param の `out X[N]` | `after` 必須。値は `primary` または他 stack 名 | declare-only スタックの base offset を anchor する。push されない top-level スタックには必須 |
| `@kunai_stack_count` | parser param の `out X[N]` | `field` 必須、`offset` | `any` / `all` quantifier 用の実行時要素数を primary の `field` 値 + `offset` で与える。`field` は byte-aligned な 8 bit |

使用例は、`variable_tail` と `writeback` を使う ipv6.p4、`layout` と `stack_count` を使う srv6.p4 にあります。
...
| `const "X" does not match <SELF>_{...}` | 命名パターン不一致。`<PARENT>` に underscore を含めた、などが典型 |
...
| `top-level declare-only aux stack "X" has no @kunai_layout` | §7.4 参照。`@kunai_layout[after=primary]` を付ける |
...
- [ ] dispatch const を親ごとに宣言した。自己検証 parser や NO_CHECK でもよいが、NO_CHECK は最終手段とする
...
const bit<16> WG_UDP_DPORT = 51820;
