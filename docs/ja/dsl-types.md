...
| `<`, `≤`, `>`, `≥` | ✅ 既存 | ✅ bracket は実装 (F3、`emitIPv6OrderedCmp` lex compare) / where-arith は staged |
| `+`, `-` | ✅ 既存 | ✅ where eq/ne 経路で `field op const` を実装 (F4、register pair carry 伝播) / `field + field` は staged |
...
| F3 | `Int<128>` ordered cmp の codegen ✅ bracket predicate 経路で実装 (`emitIPv6OrderedCmp`、lexicographic compare)。where arith 内の Int<128> ordered は引き続き staged | §9.1 |
| F4 | `Int<128>` arith binop (`+`, `-`) の codegen ⚠️ partial。`field == field` (= 単純な dual-LDX cmp) は landed、verifier-load も通過。`field op const == field` も Compile は通るが **verifier-load で permission denied** (= register-pair carry 伝播の emit が verifier の register tracking ルールに引っかかる)。実用には emit shape の追加修正が必要、現状は dslEntryExprs から外して staged | §9.1 |
...
| F8 | `field has FLAG` 実装 ✅ F6 の bitwise `&` で代替可能になったため superseded (`tcp.flags & 0x12 == 0x12` で同等)。vocab 側に flag 定数を declare するのは vocab 著者の自由 | F6 で代替 |
