...
`dsl-grammar.md` が「具象構文 (parser が食う表記)」、本書が「**抽象構文 + 型 + 意味**」、`dsl-internals.md` が「実装ノート」。3 者の関係は概ね EBNF (concrete) → AST (abstract syntax) → 型付け規則 → 操作的意味論。
...
| `ipv6.src + 1` | `Int<128>` mod 2¹²⁸ | (codegen は ErrNotImplemented、§9) |
...
| LHS | RHS | `==` `!=` | `<` `≤` `>` `≥` |
|---|---|---|---|
| `Int<N>` | `Int<M>` | ✅ widen to `Int<max(N,M)>` | ✅ 同左 (codegen は max ≤ 64 のみ; §9) |
| `Int<N>` | literal `v` | ✅ `v ⤳ Int<N>` (fit check) | ✅ 同左 (codegen は N ≤ 64 のみ) |
...
| 操作 | N ≤ 64 | 64 < N ≤ 128 |
|---|---|---|
| `==`, `!=` | ✅ 既存 (single load) | ✅ 既存 (dual load — IPv6 cmp 経路) |
| `<`, `≤`, `>`, `≥` | ✅ 既存 | ⏳ **未実装** (lexicographic cmp、~10 命令) |
| `+`, `-` | ✅ 既存 | ⏳ **未実装** (carry 伝播、~5 命令) |
| `*` | ✅ 既存 | ⏳ **未実装** (4-mul 合成、~20 命令) |
...
`feat/p4_based_dsl` ブランチで本書の D0–D10 + 残タスクを段階的に landed。代表的な commit は (新しい順):

| commit | 役割 |
|---|---|
| `d4ff30ea` | 負数リテラルの field 幅 narrow を codegen で実装 (§9.1 「Literal narrow」段落) |
| `d87caff9` | Bool ordered cmp を resolver-side でも reject (§15.4 type-soundness 完了化) |
| `0f353cd1` | resolver typing pass を `resolve/typing.go` + `typing_errors.go` に集約 (D8) |
| `6084683b` | Part II §13/§14 の hand-wave 部を補完 |
| `3d8df391` | エラーカタログ §8 の drift test (`typing_errors_test.go`) |
| `c6bbe3b7` | リテラル仕様の corner case (zone id / bracket / MAC 別形式) を test で固定 |
| `0a5f1260` | LHS network literal 対称化 (D5-3) + helper 集約 (D9-2) |
| `c990693c` | 負数リテラル受理 (D3 §4.1) |
| `59e0a287` | `flow.*` 削除 (F9) |
| `dda3e2db` | resolver の arith fit-check / static div-zero 検出 (PR-3) |
| `67f41db0` | bool atom + CIDR strict reject (PR-2) |
| `27731edd` | 仕様書 PR-1 (本書の Part I) |

PR は単一 commit ではなく、関連 commit のグループとして landed されている。詳細は `git log feat/p4_based_dsl` を参照。
...
| # | 項目 | 由来 |
|---|---|---|
| F1 | overflow lint mode | §6.1 silent wrap への安全網 |
| F2 | 明示 cast 構文 (`(bit<32>) tcp.dport`) | §5.5 |
| F3 | `Int<128>` ordered cmp の codegen | §9.1 |
| F4 | `Int<128>` arith binop (`+`, `-`) の codegen | §9.1 |
| F5 | `Int<128>` arith binop (`*`) の codegen | §9.1 |
| F6 | bitwise op (`&`, `|`, `^`, `<<`, `>>`) | DSL 機能拡張 (TCP flag check 等で需要) |
| F7 | `field in [...]` 実装 | 既存 dead syntax の有効化 |
| F8 | `field has FLAG` 実装 | 同上 |
| F9 | `flow.*` dead syntax 削除 ✅ 完了 | 整理 |
| F10 | `Bool == Bool` precision-preserving codegen | 現 desugar (§13 W-BoolEq の and/or/not 展開) は operand subtree を per-packet で 2 回評価する; 1 回評価する register-based emit に置き換える |
...
- [`dsl-followups.md`](./dsl-followups.md) — 残課題リスト (F1-F10 含む)
...
c  ::= cap(spec, w?)                                       spec ::= all | headers | headers+N | label+N | proto+N | absolute(N) | layer-target
...
──────────────────────────────────────────────
Γ ⊢ binop(op_a, e₁, e₂) : Int<max(N, M)>           ; mod 2^max wrap
```
...
Γ ⊢ e₁ : Int<N>     Γ ⊢ e₂ : Int<M>     op_c ∈ {<, ≤, >, ≥}
─────────────────────────────────────────────────────
Γ ⊢ cmp(e₁, op_c, e₂) : Bool                                   ; codegen は max(N,M) ≤ 64 のみ実装 (§9.1)
...
| T-CmpCIDR4 / T-CmpCIDR6 | `resolve/where.go::validateLiteralFieldType` |
...
- **`where false`**: filter 全体が常に reject に評価される構造的 dead-code を含むため、`internal/program/load_dsl_test.go::dslEntryExprs` から除外。Compile 自体は valid な命令列を返すので、resolver / codegen レイヤとしては仕様通り。verifier 側の chain-success-tail liveness 検査がこの形を受け付けない。F1 (overflow lint) と同様の「runtime-意味上は無害だが kernel が嫌うパターン」。本書は型 system 仕様に集中するため、このような verifier-only な制約は §15.3-3 の "structural invariants" に属する別議論。
