...
括弧は内側の式の型を保持します。`(tcp.dport) == 443` は `tcp.dport == 443` と同じ整数比較で、`(tcp.dport + 1) > 444` も整数の順序比較です。Bool への `!= 0` 変換は `where` 全体や論理演算などの Bool 文脈でだけ行います。`gtp.opt.exists == true` と `true == gtp.opt.exists` はどちらも Bool 同士の比較です。
