...
// A malformed TLV must reject a demanded option walk, even after an earlier
// option has matched. A loop iteration cap is not a substitute for rejection.
func TestTCPMalformedUnknownOptShortLength(t *testing.T) {
	...
	for _, n := range []byte{0, 1} {
		r.MustReject(t, tcpWithRawOptions(t, 7, []byte{2, 4, 5, 180, 99, n, 0, 0}, nil), "malformed TLV after matching MSS")
	}
}
